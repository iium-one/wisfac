            ...
            <form id="fm-sch" method="get">
              <div class="sch_wrap">
                <select name="sfl" id="sfl" class="nc-sel sch-select">
                  <option value="inq_subj" selected>タイトル</option>
                </select>
                <input type="text" name="stx" id="stx" class="sch-keyword" value="<?php echo $stx;?>" required placeholder="検索語を入力してください。">
                <button type="submit" class="sch-submit" title="검색하기"></button>
              </div>
              <?php if( isset($sfl) || isset($stx) ){ ?>
              <a href="/jpn/sub/contact" class="return-btn" title="전체검색"></a>
              <?php } ?>
            </form>
            <a href="/jpn/sub/contact_register" class="write-btn">問い合わせる</a>
          </div>
        </div>
      </div>
    </section>
        ...
        <div class="wrapper">
          <div class="sec_ct">
            <div class="board-list_wrap">
              ...
              if ($i == 0) {
                echo '<div class="empty_table">資料がありません。</div>';
              }
              ?>
            </div>
            <?php echo get_paging(5, $page, $total_page, '/jpn/sub/contact?'.$qstr.'&amp;page='); ?>
          </div>
        </div>
      </div>
    </section>
...
<div class="password_dim">
  <div class="password_ct">
    ...
    <p class="password_tit">パスワード確認</p>
    <p class="password_cau">
      -問い合わせ内容を作成する際に入力したパスワードを入力してください。<br/>
      パスワードを忘れた場合は閲覧できません。管理者にお問い合わせください。
    </p>
    ...
    <form id="password_fm" action="" method="post">
      ...
      <button type="submit" class="contact_check_pw_submit">閲覧する</button>
    </form>
  </div>
</div>
...
$(document).ready(function(){
  ...
  $(".view-btn").on('click', function(){
    ...
    post_link = `/jpn/sub/contact_view/${post_id}?${post_qstr}`;
