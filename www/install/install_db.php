...
$tmp_bo_table   = array ("notice", "gallery", "faq", "qa");
...
if($g5_install || !$result) {
    ...
    // 게시판 생성
    $tmp_bo_subject = array ("공지사항", "갤러리", "FAQ", "Q&A");
    for ($i=0; $i<count($tmp_bo_table); $i++)
    {
        if($tmp_bo_table[$i] === 'gallery'){
          $bo_skin = 'gallery';
        }else if($tmp_bo_table[$i] === 'faq'){
          $bo_skin = 'faq';
        }else if($tmp_bo_table[$i] === 'qa'){
          $bo_skin = 'qa';
        }else{
          $bo_skin = 'basic';
        }
